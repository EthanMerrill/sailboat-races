...
export default function SailingEventUpdateForm(props) {
  const {
    id: idProp,
    sailingEvent: sailingEventModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = sailingEventRecord
      ? { ...initialValues, ...sailingEventRecord }
      : initialValues;
  ...
  const [sailingEventRecord, setSailingEventRecord] = React.useState(
    sailingEventModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(SailingEvent, idProp)
        : sailingEventModelProp;
      setSailingEventRecord(record);
    };
    queryData();
  }, [idProp, sailingEventModelProp]);
  React.useEffect(resetStateValues, [sailingEventRecord]);
  const validations = {
    name: [{ type: "Required" }],
    link: [],
    date: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            SailingEvent.copyOf(sailingEventRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "SailingEventUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || sailingEventModelProp)}
